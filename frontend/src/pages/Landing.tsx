...
export default function Landing({ onLogin }: { onLogin: () => void }) {
  ...
  return (
    <div className="min-h-screen w-full bg-[var(--bg-primary)] text-[var(--text-primary)] relative overflow-hidden">
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <div className="absolute -top-1/4 -left-1/4 w-1/2 h-1/2 rounded-full bg-[var(--accent-glow)] blur-3xl animate-pulse" />
        <div className="absolute top-1/3 right-0 w-1/3 h-1/3 rounded-full bg-[var(--cyan-glow)] blur-3xl animate-pulse" style={{ animationDelay: '1.5s' }} />
      </div>
      ...
      <div className="relative z-10 w-full max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <section className="text-center px-6 py-20 md:py-32 max-w-4xl mx-auto">
          <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-[var(--accent-glow)] border border-[var(--accent)]/30 text-[11px] text-[var(--accent)] font-medium mb-6">
            <span className="w-1.5 h-1.5 rounded-full bg-[var(--accent)] animate-pulse" /> System Online
          </div>
          <h1 className="text-4xl md:text-6xl font-bold tracking-tight mb-6 cyber-text-glow">
          ...
          <div className="flex items-center justify-center gap-3">
            <button onClick={onLogin} className="flex items-center gap-2 px-6 py-3.5 cyber-button rounded-2xl text-sm font-bold transition-all">
              Get Started <ArrowRight className="w-4 h-4" />
            </button>
            {settings.telegram_bot_username && (
              <a href={`https://telegram.me/${settings.telegram_bot_username}`} target="_blank" rel="noopener noreferrer" className="px-6 py-3.5 bg-[var(--bg-secondary)] border border-[var(--border)] rounded-2xl text-sm font-semibold hover:bg-[var(--bg-hover)] transition-colors">
                Telegram Bot
              </a>
            )}
          </div>
        ...
        <section className="px-6 md:px-12 py-16 max-w-6xl mx-auto">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5">
            {features.map((f, i) => (
              <div key={i} className="cyber-card p-6 hover:border-[var(--accent)]/30 transition-colors">
                <div className="w-10 h-10 rounded-xl bg-[var(--accent-glow)] flex items-center justify-center mb-4">
