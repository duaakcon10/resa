...
summary() {
    ...
    echo -e "  ${GREEN}🤖 Bot WebSocket:${NC}  wss://${C2_DOMAIN}/ws/bot/<uuid>"
    echo -e "  ${GREEN}🩺 WS status:${NC}      https://${C2_DOMAIN}/api/ws/status"
    ...
    echo ""
    echo -e "  ${YELLOW}Cloudflare (bắt buộc để WS bot ổn định):${NC}"
    echo -e "  SSL/TLS → Flexible (origin chỉ HTTP:80) hoặc Full (nếu origin có TLS)"
    echo -e "  Network → WebSockets = ON  (bắt buộc)"
    echo -e "  Speed → Auto Minify: TẮT HTML/JS/CSS (có thể phá WS)"
    echo -e "  Rules: không cache /ws/*"
    echo -e "  KHÔNG bật Always Use HTTPS + origin redirect HTTPS (gây ERR_TOO_MANY_REDIRECTS)"
    echo ""
    echo -e "  ${CYAN}📋 Các lệnh hữu ích:${NC}"
    echo -e "  docker compose -f deployment/docker-compose.yml logs -f api"
    echo -e "  docker compose -f deployment/docker-compose.yml logs -f caddy"
    echo -e "  curl -s http://localhost/health && curl -s http://localhost/api/ws/status"
    echo ""
    echo -e "  ${CYAN}🤖 Deploy bot:${NC}"
    echo -e "  ./bot_static wss://${C2_DOMAIN}/ws/bot/ --foreground"
    echo ""
}
