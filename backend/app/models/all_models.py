...
class Plan(Base):
    ...
    max_attack_secs = Column(Integer, default=120)
    cooldown_secs = Column(Integer, default=300)
    max_pps_per_bot = Column(Integer, default=500000)
    allowed_methods = Column(ARRAY(Text), default=list)
    price_vnd = Column(Integer, default=10000)
    price_usd = Column(DECIMAL(10,2), default=0.5)
...
class Bot(Base):
    ...
    enabled_methods = Column(ARRAY(Text), default=lambda: ["PSPE","TCP","TLS","HTTP","GAME","MYSQL"])
...
class AttackTask(Base):
    ...
    pps_per_bot = Column(Integer, default=10000000)
    spoof_mode = Column(Integer, default=0)
    fragmentation = Column(Boolean, default=False)
